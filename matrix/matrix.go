...
*/
func isClosePath(pmoves *[]Move, bg int, end int) bool {
	moves := *pmoves
...
*/
func removeClosePath(matrix *Matrix, pmoves *[]Move) bool {
	moves := *pmoves
	for row := 0; row < len(matrix.data); row++ {
		for column := 0; column < len(matrix.data); column++ {
			if matrix.getEl(matrix.rowNums[row], matrix.columnNums[column]) == -1 {
				continue
			}
...
func h2matrix(m *Matrix, pmoves *[]Move) []Move {
	moves := *pmoves
	if m.size != 2 {
		panic("Размер матрицы не равен 2!")
	}
	if !m.reduce() {
		return moves
	}
